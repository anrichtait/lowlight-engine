/**
  ...
  Copyright 2000-2022, Morgan McGuire and Zander Majercik
  All rights reserved
...
namespace G3D {
#define TEMPLATE template<size_t INTERNAL_SIZE = 64, class Allocator = G3DAllocator<char>>
...
public:
    ...
    typedef Const_Iterator<G3DString>               const_iterator;
    typedef Iterator<G3DString>                     iterator;
    typedef std::reverse_iterator<const_iterator>   const_reverse_iterator;
    typedef std::reverse_iterator<iterator>         reverse_iterator;

protected:
    // Throw compile time error if INTERNAL_SIZE is not a multiple of SSO_ALIGNMENT
    static_assert(INTERNAL_SIZE % SSO_ALIGNMENT == 0, "G3DString Internal Size must be a multiple of 16");
...
#       endif
    ...
    constexpr inline value_type* alloc(size_t b) {
        if (b <= INTERNAL_SIZE) {
            return m_buffer;
        } else {
    ...
    constexpr value_type* prepareToMutate() {
        if (inConst()) {
            value_type* const old = m_ptr;
            ...
            value_type* dataPtr = (value_type*) alloc(m_allocatedSize);
     ...
     *  Note: Calling functions are expected to +1 for the null terminator */
    constexpr value_type* createGap(size_type newSize, size_type count, size_type count2, size_type pos) {
    ...
    /** Ensure enough bytes are allocated to hold a string of length newSize.
     *  Note: Calling functions are expected to +1 for the null terminator */
    constexpr inline value_type* maybeReallocate(size_t newSize) {
...
#if (__cplusplus < 201703L) && !defined(G3D_WINDOWS)
    ...
    ITERATOR_TRAITS
    constexpr G3DString& m_assign(InputIter first, InputIter last, std::input_iterator_tag t) {
        m_length = 0;
    ...
    ITERATOR_TRAITS
    constexpr G3DString& m_assign(InputIter first, InputIter last, std::forward_iterator_tag t){
        m_length = last - first;
        //allocate memory if necessary. 
        value_type* dataPtr = maybeReallocate(m_length + 1);
...
public:
    ...
    G3DString(std::nullptr_t): m_length(0), m_allocator(INTERNAL_SIZE) {
        m_buffer[0] = '\0';
    }

    /** Creates a zero-length string */
    constexpr inline G3DString(): m_length(0), m_allocator(INTERNAL_SIZE) {
        m_buffer[0] = '\0';
    }

    /** \param count Copy this many characters.  */
    constexpr G3DString(size_type count, value_type c) : m_length(count) {
        // Allocate more than needed for fast append
        m_allocatedSize = chooseAllocationSize(m_length + 1);
        if (inBuffer()){ 
    ...
    explicit constexpr inline G3DString(const value_type c): m_length(1), m_allocator(INTERNAL_SIZE) {
        m_buffer[0] = c;
        m_buffer[1] = '\0';
    }

    constexpr G3DString(const G3DString& str, size_type pos = 0) {
    ...
    constexpr G3DString(const G3DString& str, size_type pos, size_type count) {
    ...
    constexpr G3DString(const_pointer s) : m_length(::strlen(s)) {
        if (inConstSegment(s)) {
            m_ptr = const_cast<value_type*>(s);
            m_allocatedSize = 0;
        } else {
            // Allocate more than needed for fast append
            m_allocatedSize = chooseAllocationSize(m_length + 1);
    ...
    constexpr G3DString(const_pointer s, size_type count) : m_length(count) {
        // Allocate more than needed for fast append
        m_allocatedSize = chooseAllocationSize(m_length + 1);
        
        value_type* const dataPtr = (value_type*) alloc(m_allocatedSize);
        ::memcpy(dataPtr, s, m_length);
        dataPtr[m_length] = '\0';
    }

    constexpr G3DString(const_pointer s, size_type pos, size_type count)
        : m_length(count)
    {
        // Allocate more than needed for fast append
        m_allocatedSize = chooseAllocationSize(m_length + 1);
    ...
    constexpr G3DString(G3DString&& str) noexcept: m_length(0), m_allocator(INTERNAL_SIZE) {
        swap(str);
    }
    ...
    ITERATOR_TRAITS
    constexpr G3DString(InputIter first, InputIter last)  {
    ...
    explicit constexpr G3DString(const std::string& str, size_type pos = 0, size_type count = npos) {
        m_length = (count == npos || pos + count >= str.size()) ? str.size() - pos : count;
        // Allocate more than needed for fast append
        m_allocatedSize = chooseAllocationSize(m_length + 1);
    ...
    constexpr G3DString(std::initializer_list<value_type> ilist) : m_length(ilist.size()) {
    ...
    constexpr G3DString& operator=(const G3DString& str) {
        if (&str == this) {
            return *this;
    ...
    constexpr G3DString& operator=(G3DString&& str) noexcept {
        swap(str);
        str.clear();
        return *this;
    }

    constexpr G3DString& operator=(const_pointer s) {
        m_length = ::strlen(s);
        ...
        if (inConstSegment(s)) {
            ...
            // free and/or allocate memory if necessary. 
            value_type* const dataPtr = maybeReallocate(m_length + 1);
            // Clone the other value, putting it in the internal storage if possible
            ::memcpy(dataPtr, s, m_length + 1);
        }
        return *this;
    ...
    constexpr G3DString& operator=(const std::string& str) {
        m_length = str.length();
        // free and/or allocate memory if necessary. 
        value_type* const dataPtr = maybeReallocate(m_length + 1);
        // Clone the other value, putting it in the internal storage if possible
    ...
    constexpr G3DString& operator=(const value_type c) {
        maybeDeallocate();
        m_length = 1;
        m_allocatedSize = INTERNAL_SIZE;
        m_buffer[0] = c;
    ...
    constexpr G3DString& operator=(std::initializer_list<value_type> ilist) {
        m_length = ilist.size();
    ...
    constexpr G3DString& assign(const G3DString& str, size_type pos = 0, size_type count = npos) {
    ...
    constexpr G3DString& assign(const_pointer s, size_type count) {
        m_length = count;
        // free and/or allocate memory if necessary. 
        value_type* const dataPtr = maybeReallocate(m_length + 1);
    ...
    constexpr G3DString& assign(const_pointer s) {
        return (*this = s);
    }

    constexpr G3DString& assign(size_type count, const value_type c) {
        m_length = count;
        // free and/or allocate memory if necessary. 
        value_type* const dataPtr = maybeReallocate(m_length + 1);
    ...
    constexpr G3DString& assign(const G3DString&& str) {
        return (*this) = str;
    }

    ITERATOR_TRAITS
    constexpr G3DString& assign(InputIter first, InputIter last) {
        typedef typename std::iterator_traits<InputIter>::iterator_category tag;
        return m_assign(first, last, tag());
    }

    constexpr G3DString& assign(std::initializer_list<value_type> ilist) {
        return (*this) = ilist;
    }
    ...
    constexpr void reserve(size_type newLength = 0) {
        if (newLength > m_allocatedSize) {
            ...
            if (newLength > INTERNAL_SIZE) {
                ...
                value_type* newPtr = m_allocator.allocate(m_allocatedSize);
    ...
    constexpr G3DString& insert(size_type pos, const G3DString& str, size_type pos2, size_type count = npos) {
        if (pos == m_length) {
    ...
    constexpr G3DString& insert(size_type pos, const G3DString& str) {
        if (pos == m_length) {
            return append(str);
        }
    ...
    constexpr G3DString& insert(size_type pos, const_pointer s) {
        if (pos == m_length) {
            return append(s);
        }
    ...
    constexpr G3DString& insert(size_type pos, const_pointer s, size_type count) {
        if (pos == m_length) {
    ...
    constexpr G3DString& insert(size_type pos, size_type count, value_type c) {
        if (pos == m_length) {
    ...
    constexpr G3DString& replace(size_type pos, size_type count, const G3DString& str) {
        if (pos == m_length) {
    ...
    constexpr G3DString& replace(const_iterator first, const_iterator last, const G3DString& str) {
        if (last == end()) {
            return append(str.data(), str.m_length);
        }
    ...
    constexpr G3DString& replace(size_type pos, size_type count, const G3DString& str, size_type pos2, size_type count2 = npos) {
        return replace(pos, count, str.data() + pos2, count2);
    }

    constexpr G3DString& replace(size_type pos, size_type count, const_pointer s, size_type count2) {
        if (pos == m_length) {
        ...
        if (sizeDiff > 0) { // count < count2 -> insert
            ...
            value_type* const dataPtr = createGap(newSize, count, count2, pos); 
            ::memcpy(dataPtr + pos, s, count2);
            m_length += sizeDiff;
        } else if (sizeDiff < 0) { // count > count2 
            value_type* const dataPtr = prepareToMutate();
            ::memcpy(dataPtr + pos, s, count2);
            memmove(dataPtr + pos + count2, dataPtr + pos + count, m_length - pos - count + 1);
            m_length += sizeDiff;
        } else {
    ...
    constexpr G3DString &replace(const_iterator first, const_iterator last, const_pointer s, size_type count2) {
        if (last == end()) {
            return append(s, count2);
        }
    ...
    ITERATOR_TRAITS
    constexpr G3DString& replace(const_iterator first, const_iterator last, InputIter first2, InputIter last2) {
        const G3DString secondString (first2, last2); 
        return replace(first - begin(), last - first, secondString.data(), secondString.size());
    }

    constexpr G3DString& replace(size_type pos, size_type count, const_pointer s) {
        if (pos == m_length) {
    ...
    constexpr G3DString& replace(const_iterator first, const_iterator last, const_pointer s) {
        if (last == end()) {
            return append(s, ::strlen(s));
        }
    ...
    constexpr G3DString& replace(size_type pos, size_type count, size_type count2, value_type c) {
        if (pos == m_length) {
    ...
    constexpr G3DString& replace(const_iterator first, const_iterator last, size_type count2, value_type c) {
        if (last == end()) {
            return append(count2, c);
        }
    ...
    constexpr G3DString& replace(const_iterator first, const_iterator last, std::initializer_list<value_type> ilist) {
        if (last == end()) {
    ...
    constexpr G3DString& erase(size_type pos = 0, size_type count = npos) {
    ...
    constexpr inline friend G3DString operator+(const G3DString& lhs, const G3DString& rhs) {
        G3DString result;
        ...
        if (result.inBuffer() && lhs.inBuffer()) {
            ...
            value_type* const resultData = (value_type*) result.alloc(result.m_allocatedSize); 
    ...
    constexpr inline friend G3DString operator+(const G3DString& lhs, G3DString&& rhs) {
        return std::move(rhs.insert(0, lhs));
    }

    constexpr inline friend G3DString operator+(const G3DString& lhs, const_pointer rhs) {
    ...
    constexpr inline friend G3DString operator+(const G3DString& lhs, const value_type rhs) {
        G3DString result;
    ...
    constexpr friend G3DString operator+(const_pointer lhs, const G3DString& rhs) {
    ...
    constexpr friend G3DString operator+(const value_type lhs, const G3DString& rhs) {
        G3DString result(rhs.m_length + 1, lhs);
        ::memcpy(result.data() + 1, rhs.data(), rhs.m_length + 1);
        return std::move(result);
    }

    constexpr inline friend G3DString operator+(const_pointer lhs, G3DString&& rhs) {
        return std::move(rhs.insert(0, lhs));
    }

    constexpr inline friend G3DString operator+(const value_type lhs, G3DString&& rhs) {
        return std::move(rhs.insert(0, 1, lhs));
    }

    constexpr inline friend G3DString operator+(G3DString&& lhs, G3DString&& rhs) {
        return std::move(G3DString(lhs).append(rhs));
    }

    constexpr inline friend G3DString operator+(G3DString&& lhs, const G3DString& rhs) {
        return std::move(lhs.append(rhs));
    }

    constexpr inline friend G3DString operator+(G3DString&& lhs, const_pointer rhs) {
        return std::move(lhs.append(rhs));
    }

    constexpr inline friend G3DString operator+(G3DString&& lhs, const value_type rhs) {
        return std::move(lhs.append(1, rhs));
    }

    constexpr G3DString& operator+=(const G3DString& str) {
        value_type* dataPtr = ensureAllocation(m_length + str.m_length + 1);
    ...
    constexpr G3DString& operator+=(const value_type c) {
        ...
        value_type* const dataPtr = ensureAllocation(m_length + 2);
        dataPtr[m_length++] = c;
        dataPtr[m_length] = '\0';
        return *this;
    }
    ...
    constexpr G3DString& operator+=(const_pointer s) {
        ...
        value_type* dataPtr = ensureAllocation(m_length + t + 1); 
    ...
    constexpr G3DString& append(const G3DString& str, size_type pos, size_type count = npos) {
        size_type copy_len = (count == npos || pos + count >= str.size()) ? str.size() - pos : count;
        value_type* const dataPtr = ensureAllocation(m_length + copy_len + 1);
        ::memcpy(dataPtr + m_length, str.data() + pos, copy_len);
        dataPtr[m_length += copy_len] = '\0';
        return *this;
    }

    constexpr G3DString& append(const G3DString& str) {
        return (*this += str);
    }

    constexpr G3DString& append(size_type count, value_type c) {
        value_type* const dataPtr = ensureAllocation(m_length + count + 1);
        ::memset(dataPtr + m_length, c, count);
        dataPtr[m_length += count] = '\0';
        return *this;
    }

    constexpr G3DString& append(const_pointer s, size_type t) {
        value_type* const dataPtr = ensureAllocation(m_length + t + 1);
        ::memcpy(dataPtr + m_length, s, t);
        dataPtr[m_length += t] = '\0';
        return *this;
    }

    constexpr G3DString& append(const_pointer s) {
        return (*this) += s;
    }

    ITERATOR_TRAITS
    constexpr G3DString& append(InputIter first, InputIter last) {
        const G3DString secondString(first, last);
        return this->append(secondString);
    }

    constexpr G3DString& append(std::initializer_list<value_type> ilist) {
        return this->append(ilist.begin(), ilist.size());
    }

    constexpr void swap(G3DString& str) {
    ...
    constexpr size_type find(const G3DString& str, size_type pos = 0) const {
    ...
    constexpr size_type rfind(const G3DString& str, size_type pos = npos) const {
    ...
    constexpr size_type find_first_of(const G3DString& str, size_type pos = 0) const {
    ...
    constexpr size_type find_first_not_of(const G3DString& str, size_type pos = 0) const {
    ...
    constexpr size_type find_last_of(const G3DString& str, size_type pos = npos) const {
    ...
    constexpr size_type find_last_not_of(const G3DString& str, size_type pos = npos) const {
...
public:
    ...
    constexpr int compare(const G3DString& str) const {
        const_pointer const dataPtr = data(); 
    ...
    constexpr int compare(size_type pos, size_type count, const G3DString& str) const {
        return m_compare(data() + pos, std::min(m_length - pos, count), str.data(), str.m_length);
    }

    constexpr int compare(size_type pos, size_type count1, const G3DString& str, size_type pos2, size_type count2) const {
    ...
    friend constexpr inline bool operator==(const_pointer s, const G3DString& str) {
        return str == s;
    }

    constexpr inline bool operator==(const G3DString& str) const {
    ...
    constexpr inline bool equals(const G3DString& str) const {
        return ((m_length == str.m_length) && (data() == str.data())) || !m_compare(data(), m_length, str.data(), str.m_length);
    }

    constexpr inline bool operator!=(const G3DString& s) const {
        return !(*this == s);
    }
    ...
    constexpr bool operator>(const G3DString& s) const {
        return compare(s) > 0;
    }
...
TEMPLATE
std::ostream& operator<<(std::ostream& os, const G3DString<INTERNAL_SIZE, Allocator>& str) {
    ...
    if (sen) {
        try {
            ...
            if (w > (std::streamsize) str.size()) {
                ...
                if (!left) {    
                    const typename G3DString<INTERNAL_SIZE, Allocator>::value_type c = os.fill();
...
TEMPLATE
std::istream& operator>>(std::istream& is, G3DString<INTERNAL_SIZE, Allocator>& str) {
    typename G3DString<INTERNAL_SIZE, Allocator>::size_type numExtracted = 0;
    ...
    if (sen) {
        ...
        {
            str.erase();
            const typename G3DString<INTERNAL_SIZE, Allocator>::size_type n =
                is.width() > 0 ? static_cast<typename G3DString<INTERNAL_SIZE, Allocator>::size_type>(is.width())
                    : str.max_size();
            typename G3DString<INTERNAL_SIZE, Allocator>::value_type c = is.rdbuf()->sgetc();
...
TEMPLATE
std::istream& getline(
    std::istream& is, G3DString<INTERNAL_SIZE, Allocator>& str, typename G3DString<INTERNAL_SIZE, Allocator>::value_type delim = '\n') {
    typename G3DString<INTERNAL_SIZE, Allocator>::size_type numExtracted = 0;
    std::istream::ios_base::iostate  err = std::istream::ios_base::goodbit;
    ...
    if (sen) {
        ...
        {
            str.erase();
            const typename G3DString<INTERNAL_SIZE, Allocator>::size_type n = str.max_size();
            typename G3DString<INTERNAL_SIZE, Allocator>::value_type c = is.rdbuf()->sgetc();
...
TEMPLATE 
inline int stoi(
    const G3DString<INTERNAL_SIZE, Allocator> &str, typename G3DString<INTERNAL_SIZE, Allocator>::size_type* pos = nullptr, int base = 10) {
    typename G3DString<INTERNAL_SIZE, Allocator>::value_type* end;
...
TEMPLATE 
inline float stof(
    const G3DString<INTERNAL_SIZE, Allocator> &str, typename G3DString<INTERNAL_SIZE, Allocator>::size_type* pos = nullptr) {
    typename G3DString<INTERNAL_SIZE, Allocator>::value_type* end;
...
template<size_t INTERNAL_SIZE = 64, class Allocator = G3DAllocator<char>, typename IntType>
G3DString<INTERNAL_SIZE, Allocator>  int_to_string(IntType value) {
    const int n = std::numeric_limits<IntType>::digits10 + 3;
    char str[n + 1] = {'\0'};

    if (std::is_unsigned<IntType>::value || value >= 0) {
        return G3DString<INTERNAL_SIZE, Allocator>(uint_to_buffer(str + n, value));
    } else {
        using UIntType = std::make_unsigned_t<IntType>;
        char* start  = uint_to_buffer(str + n, static_cast<UIntType>(0 - value));
        *(--start) = '-';

        return G3DString<INTERNAL_SIZE, Allocator>(start);
    }
}

TEMPLATE 
G3DString<INTERNAL_SIZE, Allocator> to_string(int value) {
    return int_to_string(value);
}

TEMPLATE 
G3DString<INTERNAL_SIZE, Allocator> to_string(long value) {
    return int_to_string(value);
}

TEMPLATE 
G3DString<INTERNAL_SIZE, Allocator> to_string(long long value) {
    return int_to_string(value);
}

TEMPLATE 
G3DString<INTERNAL_SIZE, Allocator> to_string(unsigned int value) {
    return int_to_string(value);
}

TEMPLATE 
G3DString<INTERNAL_SIZE, Allocator> to_string(unsigned long value) {
    return int_to_string(value);
}

TEMPLATE 
G3DString<INTERNAL_SIZE, Allocator> to_string(unsigned long long value) {
    return int_to_string(value);
}

TEMPLATE 
G3DString<INTERNAL_SIZE, Allocator> to_string(float value) {
    ...
    typename G3DString<INTERNAL_SIZE, Allocator>::value_type str[n];
    snprintf(str, n, "%f", value);
    return G3DString<INTERNAL_SIZE, Allocator>(str);
}

TEMPLATE 
G3DString<INTERNAL_SIZE, Allocator> to_string(double value) {
...
TEMPLATE 
G3DString<INTERNAL_SIZE, Allocator> to_string(long double value) {
...
TEMPLATE 
typename G3DString<INTERNAL_SIZE, Allocator>::iterator begin(G3DString<INTERNAL_SIZE, Allocator>& str) {
    return str.begin();
}

TEMPLATE
typename G3DString<INTERNAL_SIZE, Allocator>::iterator end(G3DString<INTERNAL_SIZE, Allocator>& str) {
    return str.end();
}
...
#else
    // Fastest
    typedef G3DString<64, G3DAllocator<char>> String;
