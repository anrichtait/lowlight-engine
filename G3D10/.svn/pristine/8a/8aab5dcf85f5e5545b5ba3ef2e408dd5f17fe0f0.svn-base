...
Color4 Texture::readTexel(int x, int y, RenderDevice* rd, int mipLevel, int z, CubeFace face) const {
    ...
    if (format()->isIntegerFormat()) {
        ...
        Color4 c;
        if (texelType() == TexelType::UNSIGNED_INTEGER) {
            unsigned int* uints = reinterpret_cast<unsigned int*>(ints);
            c = Color4(float(uints[0]), float(uints[1]), float(uints[2]), float(uints[3]));
        }
        else {
...
#   else
    {
        debugAssertM(GLCaps::supports("GL_ARB_clear_texture"), "OpenGL version should be at least 4.3");
