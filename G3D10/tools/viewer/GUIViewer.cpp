...
void GUIViewer::onInit(const G3D::String& filename, App* app) {
    createGui(filename);
}


void GUIViewer::onGraphics3D(RenderDevice* rd, Array<shared_ptr<Surface> >& surfaceArray) {
    switch (windowControl) {
    case WHITE:
        parentApp->colorClear = Color3::white();
        break;
    case BLUE:
        parentApp->colorClear = Color3::blue();
        break;
    case BLACK:
        parentApp->colorClear = Color3::black();
        break;
